...
\exercise \marginpar{\small{\textit{\href{https://math.hws.edu/eck/cs124/csharpnotes1/c<xsl:number count="chapter"/>/ex<xsl:number count="exercise"/>-ans.html}{\ \ \ (solution)}}}}\ignorespaces <xsl:apply-templates select="exercise-question"/>
</xsl:for-each> 
...
\begin{quiz}\marginpar{\small{\textit{\href{https://math.hws.edu/eck/cs124/csharpnotes1/c<xsl:number count="chapter"/>/quiz_answers.html}{\ \ (answers)}}}}
...
<!-- 
<xsl:template match="start">\<xsl:value-of select="name()"/>{<xsl:apply-templates/>}<xsl:if test="ancestor::section">\marginpar{\small{\textit{\href{https://math.hws.edu/eck/cs124/csharpnotes1/c<xsl:number count="chapter"/>/s<xsl:number count="section"/>.html }{\ \ \ (online)}}}}</xsl:if></xsl:template>
-->
...
<xsl:template match="sourceref"><xsl:variable name="a" select="str:new(@href)"/><xsl:variable name="b" select="str:replaceAll($a,'_','\\_')"/>
   ...
   <xsl:variable name="ref">
      <xsl:choose>
         <xsl:when test="ancestor::source"><xsl:text>https://math.hws.edu/eck/cs124/csharpnotes1/source/</xsl:text><xsl:value-of select="@href"/></xsl:when>
...
<xsl:template match="jarref"><xsl:variable name="a" select="str:new(@href)"/><xsl:variable name="b" select="str:replaceAll($a,'_','\\_')"/><xsl:choose>
   <xsl:when test="text()">\href{https://math.hws.edu/eck/cs124/csharpnotes1/jars/c<xsl:number count="chapter"/>/<xsl:value-of select="$b"/>}{<xsl:apply-templates/>}</xsl:when>
   <xsl:otherwise>\href{https://math.hws.edu/eck/cs124/csharpnotes1/jars/chapter<xsl:number count="chapter"/>/<xsl:value-of select="$b"/>}{<xsl:value-of select="$b"/>}</xsl:otherwise>
</xsl:choose></xsl:template>
