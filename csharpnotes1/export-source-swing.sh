...
# that contains this script!).  csharpnotes1 should be correct.
...
SOURCE_DIR='/home/eck/git/csharpnotes1'
...
#Remove source folder src-extra, which contains duplicates of files from csharpnotes1 that are
#also needed in csharpnotes1-swing

rm -r $PROJECT/src-extra

perl -i -p -e 's/\t/    /g' `find . -name "*.java"`
perl -i -p -e 's/<!DOCTYPE.*csharpnotes1.dtd" *>//' `find . -name "*.xml" -and ! -name "csharpnotes1*"`
