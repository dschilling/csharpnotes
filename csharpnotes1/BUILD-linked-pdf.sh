...
# THIS SCRIPT BUILDS THE LINKED CSHARPNOTES PDF FILE (the version WITH links).
...
if ! $XALAN_COMMAND -xsl convert-tex-linked.xsl -in csharpnotes1-tex.xml -out tex/csharpnotes1-linked.tex ; then
   echo
...
$LATEX_COMMAND csharpnotes1-linked.tex
$LATEX_COMMAND csharpnotes1-linked.tex
$LATEX_COMMAND csharpnotes1-linked.tex

if [ ! -e "csharpnotes1-linked.dvi" ] ; then
   echo
   echo "An error occurred while trying to run latex on csharpnotes1-linked.tex."
...
$DVIPDF_COMMAND csharpnotes1-linked.dvi

if [ ! -e "csharpnotes1-linked.pdf" ] ; then
   echo
   echo "An error occurred while trying to run dvipdf on csharpnotes1-linked.dvi."
   echo "Exiting from BUILD-PDF.sh; latex files are in $JAVA_SOURCE_DIR/tex"
   echo
   cd ..
   exit 1
...
if ! mv csharpnotes1-linked.pdf $BUILD_OUTPUT_DIR ; then
...
echo "csharpnotes1-linked.pdf created in $BUILD_OUTPUT_DIR"
